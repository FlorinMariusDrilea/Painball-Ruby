...
#declare Array to read every blank space
number=Array.new(15,15)
...
#initialise a 2D array to store how many visits in each cell starting with 0
$countArray = Array.new(SCREEN_Y) { Array.new(SCREEN_X, 0)}

#initialise a counter to store the total visits of the ball in all cells
$counter= 0
...
def update(racquet,screen,testmode)
    ...
    #the ball will bounce in the other direction if it hits wall "|"
    if screen[$y.floor][$x.floor]==SC_V
        $dx=-$dx
    
    #check what the screen array contains and modify accordingly
    elsif screen[$y.floor][$x.floor]==SC_BLANK ||  screen[$y.floor][$x.floor]==SC_STAR  
        return x==$x.floor && y==$y.floor

    #the ball will bounce in other direction and going down if it hits upside "-"
    elsif screen[$y.floor][$x.floor]==SC_H
        $dy=-$dy 
        
    #the ball will bounce up if it hits the racquet    
    elsif screen[$y.floor][$x.floor]==SC_H && ($x.floor==racquet || $x.floor==racquet-1)
        $dy=-$dy 
        
    #the ball take random direction when hits the racquet off-centre by 1
    elsif screen[$y.floor][$x.floor]==SC_H && ($x.floor==racquet+1 || $x.floor==racquet-2) 
        $dx=rand(-0.3..0.3)
        $dy=-$dy
        speed=Math.sqrt($dx*$dx+$dy*$dy)
        $dy=-speed.abs
        
    #the ball will keep its coordinates if it is on blank and doesn't hit anything   
    elsif screen[$y.floor][$x.floor]==SC_BLANK
        return x==$x.floor && y == $y.floor
        
    #if the ball is not catch down with the racquet the game will be over    
    else 
        return nil 
    end
...
def displayDyn(screen,racquet)
    ...
    #$counter is initialised at 0, increase by 1 everytime this function is called
    #to calculate the total number of visits in all the cells
    $counter+=1
    p=2/100*$counter
    #print stars in the blank cells 
    #the cells in countArray increase by 1 everytime the ball passes through them
    #use oldx, oldy because the counter increments after the ball passes so the old coordinates
    #of the ball need to be taken into account.
    if screen[$oldy.floor][$oldx.floor]==SC_BLANK
        $countArray[$oldy.floor][$oldx.floor]+=1
            #check whether the counter in each cell is the 2% of the total counter of the ball and if it is more than 3 times
            if $countArray[$oldy.floor][$oldx.floor]>p &&  $countArray[$oldy.floor][$oldx.floor]>3
...
def racquetRight(racquet)
    # check that it doesn't overwrite the walls
    if (racquet+RACQUET_SIZE-1)<SCREEN_X 
        print "\e[#{SCREEN_Y};#{racquet-1}H"
        print SC_H
        print "=" *RACQUET_SIZE
        #racquet increases by 1 each time
        racquet=racquet+1
    else 
        racquet
    end
...
def displayEndgame
    print " "
 	puts "\e[#{SCREEN_Y+1};#{1}HGame over.\e[#{SCREEN_Y+1};#{1}H"
end
